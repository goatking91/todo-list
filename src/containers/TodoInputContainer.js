...
export default connect(
  ...
  (dispatch) => ({
    /*  bindActionCreators를 사용하면 자동으로 다음 작업들을 합니다.
        {
          actionCreator: (...params) => dispatch(actionCreater(...params))
        }
        그래서 이전에 우리가 했던 것처럼 일일이 dispatch할 필요가 없습니다.
        예를 들어 InputActions에는 다음 작업이 되어 있는 것이죠.
        InputActions: {
          setInput: (value) => dispatch(inputAction.setInput(value))
        }
        나중에 이를 호출할 때는 this.props.InputAction.setInput을 호출하면 됩니다.
    */
